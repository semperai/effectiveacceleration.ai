...
contract TreasurySplit {
    ...
    event FundsDistributed(
        uint256 amount,
        address indexed eaccTreasuryAddress,
        address indexed arbiusTreasuryAddress
    );
    ...
    constructor(
        ...
        require(
            _eaccTreasuryAddress != address(0),
            "Invalid EACC treasury address"
        );
        require(
            _arbiusTreasuryAddress != address(0),
            "Invalid arbius treasury address"
        );
        require(
            _eaccTreasuryPercentage <= 1 ether,
            "Percentage must be between 0 and 1 ether"
        );
    ...
    function split(IERC20 _token) external {
        ...
        uint256 eaccTreasuryAmount = (balance * eaccTreasuryPercentage) /
            1 ether;
