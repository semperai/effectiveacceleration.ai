...
contract UnicrowDispute is IUnicrowDispute, Context, ReentrancyGuard {
    ...
    function challenge(uint256 escrowId) external override nonReentrant {
        ...
        require(
            sender != escrow.buyer || escrow.consensus[WHO_BUYER] <= 0,
            "1-014"
        );

        // Prevent reduntant challenge from buyer's side
        require(
            sender != escrow.seller || escrow.consensus[WHO_SELLER] <= 0,
            "1-015"
        );
        ...
        if (sender == escrow.buyer) {
            ...
            escrow.consensus[WHO_SELLER] = -(
                abs8(escrow.consensus[WHO_SELLER])
            );
        ...
        uint64 periodEnd = escrow.challengePeriodEnd +
            escrow.challengeExtension;
    ...
    function approveSettlement(
        ...
        address latestSettlementOfferByAddress = latestSettlementOfferBy[
            escrowId
        ];
        ...
        unicrow.settle(escrowId, split, escrow.consensus);
    ...
    function getSettlementDetails(
        ...
        Settlement memory settlement = Settlement(
            latestSettlementOfferBy[escrowId],
            latestSettlementOffer[escrowId]
        );
        return settlement;
    }
}
