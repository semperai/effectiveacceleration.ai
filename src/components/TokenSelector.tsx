...
function TokenButton({ onClick, selectedToken }: {
  onClick: () => void
...
function TokenListItem({ token, selected = false, onClick }: {
  token: Token;
  selected: boolean;
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col items-start">
            ...
            <div className="text-sm font-medium text-slate-400">{token.symbol}</div>
          </div>
        </div>
        <div>
...
export function TokenSelector({ selectedToken, onClick }: {
  selectedToken: Token | undefined
  onClick: (token: Token) => void
}) {
