...
const ActionSection = ({ 
  ...
  const variantStyles = {
    default: 'from-gray-50 to-gray-100 dark:from-gray-800/30 dark:to-gray-900/30 border-gray-200 dark:border-gray-700',
    warning: 'from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-amber-200 dark:border-amber-800',
    success: 'from-emerald-50 to-green-50 dark:from-emerald-950/20 dark:to-green-950/20 border-emerald-200 dark:border-emerald-800',
  ...
  return (
    <div className={`
      relative rounded-xl bg-gradient-to-br ${variantStyles[variant]}
      border
...
const JobButtonActions = ({
  ...
  if (showUpdateButton) ownerActions.push(
    <UpdateButton key="update" address={address} job={job} />
  );
  if (showCloseButton) ownerActions.push(
    <CloseButton key="close" job={job} />
  );
  if (showReopenButton) ownerActions.push(
    <ReopenButton key="reopen" job={job} />
  );
  if (showReviewButton) ownerActions.push(
    <ReviewButton key="review" address={address} job={job} />
  );
  if (showWithdrawCollateralButton) ownerActions.push(
    <WithdrawCollateralButton key="withdraw" job={job} />
  );
  if (showWhitelistButton) ownerActions.push(
    <WhitelistButton key="whitelist" address={address} job={job} whitelist={whitelistedWorkers} />
  );
  if (showRemoveFromWhitelistButton) ownerActions.push(
    <RemoveFromWhitelistButton key="remove-whitelist" address={address} job={job} whitelist={whitelistedWorkers} />
  );

  // Populate worker actions
  if (showAcceptButton) workerActions.push(
    <AcceptButton key="accept" address={address} job={job} events={events} />
  );
  if (showDeliverResultButton) workerActions.push(
    <DeliverResultButton key="deliver" address={address} job={job} sessionKeys={sessionKeys} />
  );
  if (showRefundButton) workerActions.push(
    <RefundButton key="refund" job={job} />
  );

  // Populate arbitrator actions
  if (showArbitrateButton) arbitratorActions.push(
    <ArbitrateButton key="arbitrate" address={address} job={job} sessionKeys={sessionKeys} />
  );
  if (showRefuseArbitrationButton) arbitratorActions.push(
    <RefuseArbitrationButton key="refuse" job={job} />
  );
  ...
  return (
    <div className='jobButtonActions space-y-3'>
      ...
      {ownerActions.length > 0 && (
        <ActionSection title="Owner Actions" icon={PiUserPlus} variant="default">
          {ownerActions}
        </ActionSection>
      )}
      ...
      {arbitratorActions.length > 0 && (
        <ActionSection title="Arbitrator Actions" icon={PiScales} variant="warning">
          {arbitratorActions}
        </ActionSection>
      )}
