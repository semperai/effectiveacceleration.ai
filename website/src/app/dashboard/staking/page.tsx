...
import { StreamsPanel } from './StreamsPanel'; // We'll create this component

export default function StakingPage() {
  const { showError } = useToast();
  const [activeTab, setActiveTab] = useState<'staking' | 'streams'>('staking');
          ...
          <div className="max-w-2xl mx-auto w-full bg-white p-8 rounded-2xl shadow-xl">
            ...
            <div className="flex justify-center mb-8">
              <div className="flex bg-gray-100 rounded-lg p-1">
                <button
                  className={`px-4 py-2 rounded-md ${activeTab === 'staking' ? 'bg-blue-500 text-white' : 'text-gray-700'}`}
                  onClick={() => setActiveTab('staking')}
                >
                  Staking
                </button>
                <button
                  className={`px-4 py-2 rounded-md ${activeTab === 'streams' ? 'bg-blue-500 text-white' : 'text-gray-700'}`}
