...
import { getMediaFromIpfs, publishMediaToIpfs } from '@effectiveacceleration/contracts';
...
const Test = () => {
  ...
  const publish = useCallback(async (file: File) => {
    ...
    const { cid } = await publishMediaToIpfs(mimeType, new Uint8Array(data), sessionKey);
    const downloadHash = '#'+encodeURIComponent(`filename=${file.name}&cid=${cid}${sessionKey ? `&sessionKey=${sessionKey}` : ''}`);
  ...
  return (
    <Layout>
      ...
      <div className='border-solid border-4 p-5'>
        ...
        <div>
          <Input type="file" id="file" name="file" onChange={(e) => publish(e.target.files?.[0]!)} />
        </div>

        {downloadHash &&
          <a href={downloadHash}>download link</a>
        }

        <div className='grid grid-cols-2 gap-20 my-5 h-52'>
          <div>
            <Textarea value={markdownContent} className="h-full leading-6" onChange={(e) => setMarkdownContent(e.target.value)}>
              {markdownContent}
            </Textarea>
          </div>
          <div>
