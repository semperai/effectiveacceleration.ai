...
export default function ComboboxType ({data, selected, setSelected}: {data: { id: number; name: string; }[], selected: { id: number; name: string; }[], setSelected: (value: { id: number; name: string; }[]) => void}): JSX.Element {
  ...
  return (
      <Combobox multiple value={selected} onChange={(value: { id: number; name: string; }[]) => setSelected(value)}>
        <div className="relative group">
          ...
          />
          <Combobox.Button className="group absolute b-10 inset-y-0 right-0 px-2.5">
        ...
        <Combobox.Options
          ...
          // transition
          className={clsx(
            'lg:min-w-[400px] rounded-xl border border-white/5 bg-white p-1 [--anchor-gap:var(--spacing-1)] empty:invisible z-50 shadow-lg',
        ...
        >
          {filteredData.map((value: { id: number; name: string }) => (
            <Combobox.Option
              key={value.id}
              value={value}
