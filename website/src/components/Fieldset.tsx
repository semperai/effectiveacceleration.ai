...
export function FieldGroup({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return <div {...props} data-slot="control" className={clsx(className, 'space-y-5')} />
}
...
export function Label({ className, ...props }: { className?: string } & HeadlessLabelProps) {
  return (
    <HeadlessLabel
      ...
      className={clsx(
        className,
        'select-none text-base/6 !font-bold text-darkBlueFont data-[disabled]:opacity-50 sm:text-sm/6 dark:text-white mb-1'
      )}
    />
  )
}
