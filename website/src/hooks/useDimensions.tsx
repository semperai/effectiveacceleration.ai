...
function useDimensions(ref: RefObject<HTMLElement>): Dimensions {
  const getServerSnapshot = (): string => JSON.stringify({
  ...
  const dimensions = useSyncExternalStore(
    ...
    (): string => JSON.stringify({
      width: typeof window !== "undefined" ? ref.current?.offsetWidth ?? 0 : 0,
      height: typeof window !== "undefined" ? ref.current?.offsetHeight ?? 0 : 0,
    }),
